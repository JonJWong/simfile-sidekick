...
FONT_BOLD = ImageFont.truetype("assets/font/DejaVuSansMono-Bold.ttf",
                               FONT_SIZE)
...
FONT_LINE_SPACING = math.floor(
    CHAR_HEIGHT * 1.2)  # Add a small space between lines for readability
...
def save_image(image: Image, path: str) -> bool:
    ...
    try:
        ...
        logging.error(
            "The image '{}' doesn't appear to be a valid Image object.".format(
                image),
            exc_info=True)
        return False
    except OSError as e:
        logging.error("The file '{}' could not be saved.".format(path),
                      exc_info=True)
        return False
...
def load_image(path: str) -> Image:
    ...
    try:
        ...
        logging.error("Unable to locate image file '{}'.".format(path),
                      exc_info=True)
        return None
    except UnidentifiedImageError as e:
        logging.error(
            "Unable to load image file '{}'. Is this an image file, or is the image corrupt?"
            .format(path),
            exc_info=True)
        return None


def create_and_save_density_graph(x: List[int], y: List[float],
                                  path: str) -> bool:
    ...
    fig = go.Figure(
        go.Scatter(x=x,
                   ...
                   line_color="orange",
                   line=dict(width=0.5)))
    fig.update_layout(plot_bgcolor=BG_RGB,
                      paper_bgcolor=BG_RGB,
    ...
    try:
        ...
        logging.error(
            "The density graph could not be saved at '{}'.".format(path),
            exc_info=True)
        return False
